...
let inventorNames = inventors.map(inventor => {
    return `First Name: ${inventor.first}, Last Name: ${inventor.last}` 
})
...
let sortedByBirthYear = inventors.toSorted((a, b) => a.year - b.year)
...
// const reformattedPeople = people.map(person => { // Split the string into an array of two elements [Last, First] const [last, first] = person.split(', '); // Rearrange the elements to [First, Last] and join them with a space return `${first} ${last}

// const checkThis = people.map(() => {
//     console.log(`first ${peopleNamesArr[0]} last ${peopleNamesArr[1]}`)
// })



// const unorderedStrArray = people.toString()
// const vaguelyOrderedStrArray = unorderedStrArray.split(", ")
// console.log(vaguelyOrderedStrArray)
